...
probes:
  ...
  second_opinion: python3 -m validated_memory.probes.second_opinion
---
...
- `probes` maps an anchor `kind` to the command that probes it for
  freshness. Two ship with the plugin:

  - `git_ref` -- resolves a repository ref and compares it with what was
    captured.
  - `second_opinion` -- asks a *different* model whether a claim still holds.
    A model reviewing its own output shares its own blind spots; a different
    one is a genuinely independent check.

    🔴 Its answer is an OPINION, never a measurement. `drifted` here means
    another model disagrees, which is a reason to go and check, not proof.
    **It can never be the basis for calling a unit `measured`.** If it is your
    only evidence, the honest state is `hypothesis`.

    Needs `VALIDATED_MEMORY_MODEL_URL` and `VALIDATED_MEMORY_MODEL_NAME` in the
    environment; without them it answers `unknown`, which is the honest
    answer.
